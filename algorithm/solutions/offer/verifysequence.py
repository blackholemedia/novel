    ...
    def verify_sequence_of_BST(self, sequence):
        if sequence:
            if len(sequence) < 3:
                ...
                return self.verify_sequence_of_BST(sequence[:-1])
                ...
                if reduce(lambda x,y:(x and sequence[-1] > y) if isinstance(x,bool) else (sequence[-1] > x and sequence[-1] > y),sequence[:root_index]) and reduce(lambda x,y:(x and sequence[-1] < y) if isinstance(x,bool) else (sequence[-1] < x and sequence[-1] < y),sequence[root_index:-1]):
                    return self.verify_sequence_of_BST(sequence[:root_index]) and self.verify_sequence_of_BST(sequence[root_index:-1])
                else:
                    return False
            else:
                return False
...
if __name__ == '__main__':
    ...
    '''
    ytree = Solution()
    print(ytree.verify_sequence_of_BST(randomlist))
